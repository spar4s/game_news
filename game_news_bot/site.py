...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .hero {{
      background: linear-gradient(135deg, #16365f 0%, #285ea8 100%);
      color: white;
      border-radius: 28px;
      padding: 30px;
...
def _render_homepage(
    ...
    stats = {
        ...
        "high_heat_count": len([row for row in topics if row["importance_score"] >= 90]),
        "source_count": len({row["source_name"] for row in topics if row["source_name"]}),
    }
    filter_buttons = "".join(
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    body {{
      ...
      font-family: "Segoe UI", "PingFang SC", "Microsoft YaHei", sans-serif;
      background:
        radial-gradient(circle at 0% 0%, rgba(31,111,235,0.12), transparent 24%),
        radial-gradient(circle at 100% 12%, rgba(31,111,235,0.08), transparent 18%),
    ...
    .brand-badge {{
      ...
      color: #fff;
      font-weight: 800;
      background: linear-gradient(135deg, #0f2542 0%, #2a5fa8 100%);
    }}
    ...
    .hero {{
      ...
      background:
        ...
        linear-gradient(135deg, #16365f 0%, #285ea8 100%);
      box-shadow: var(--shadow);
    }}
    ...
    .panel, .section-card, .hero-card, .story, .bulletin-card {{
      background: var(--surface);
      border: 1px solid var(--line);
    ...
    .lead-grid {{ display: grid; grid-template-columns: 1fr; gap: 16px; }}
    .content-stack {{ display: grid; gap: 22px; }}
    .bulletin-grid, .section-list {{ display: grid; gap: 14px; }}
    .hero-card, .story, .bulletin-card {{
      position: relative;
      overflow: hidden;
      padding: 18px 18px 16px;
      transition: transform 180ms ease, box-shadow 180ms ease, border-color 180ms ease;
    }}
    .hero-card:hover, .story:hover, .bulletin-card:hover {{
      transform: translateY(-2px);
      border-color: rgba(31,111,235,0.26);
      box-shadow: 0 28px 68px rgba(25, 52, 88, 0.14);
    }}
    .card-topline {{
      height: 4px;
      margin: -18px -18px 14px;
      background: linear-gradient(90deg, var(--card-accent, #1f6feb), transparent 90%);
    }}
    .pick-badge {{
      position: absolute;
      top: -1px;
      ...
      background: linear-gradient(135deg, #ffb24a 0%, #ff8b2b 100%);
      color: #fff;
      font-size: 12px;
      font-weight: 700;
    ...
    .news-avatar {{
      ...
      display: grid;
      place-items: center;
      background: var(--avatar-bg, linear-gradient(145deg, #0e1726 0%, #29354d 100%));
      color: var(--avatar-fg, #fff);
      font-weight: 800;
      font-size: 20px;
      box-shadow: inset 0 1px 0 rgba(255,255,255,0.2), 0 10px 24px rgba(16, 34, 60, 0.12);
    }}
    ...
    .score-pill {{
      ...
      padding: 7px 10px;
      border-radius: 12px;
      border: 1px solid rgba(255, 172, 73, 0.5);
      background: linear-gradient(180deg, rgba(255, 251, 236, 0.92), rgba(255, 239, 199, 0.88));
      color: #b56a0f;
      font-size: 13px;
      font-weight: 700;
    }}
    ...
    .tag-row {{
      ...
      margin-top: 14px;
      padding-top: 12px;
      border-top: 1px solid rgba(138,160,187,0.18);
    }}
    .tag {{
      display: inline-flex;
      align-items: center;
      padding: 6px 11px;
      border-radius: 10px;
      border: 1px solid rgba(126, 147, 176, 0.22);
      background: #f8fbff;
      color: #5f7492;
      font-size: 12px;
      line-height: 1;
    ...
    @media (max-width: 700px) {{
      ...
      .news-avatar {{ width: 46px; height: 46px; border-radius: 14px; }}
...
<body>
  <div class="wrap">
    ...
    <section class="hero">
      ...
      <div class="hero-copy">{escape(site_date)} 的游戏资讯首页。这里把今日导读、热点快讯、来源结构和栏目内容整理成更像正式站点的单页，适合本地预览，也方便后续直接部署成静态网站。</div>
      <div class="stats">
    ...
    <div class="layout">
      <aside class="sidebar">
        ...
        <section class="panel">
          <h3>历史归档</h3>
          <ul class="archive-list">{archive_list}</ul>
        </section>
        <section class="panel">
          <h3>来源榜单</h3>
          <ul class="source-list">{source_list}</ul>
        </section>
        <section class="panel">
          <h3>页面说明</h3>
          <p>左侧可以按栏目筛选，也可以直接搜索具体游戏名。主内容区改成资讯流卡片，阅读路径会更接近真正的媒体站首页。</p>
        </section>
      </aside>
      <main class="content-stack">
        <section id="lead" class="section-card" data-section="导读">
          <h2>今日导读</h2>
          <div class="lead-grid">
            {''.join(_render_lead_item(row) for row in lead) or '<article class="hero-card"><p>今天还没有导读内容。</p></article>'}
          </div>
        </section>
        <section id="bulletins" class="section-card" data-section="快讯">
          <h2>热点快讯</h2>
          <div class="bulletin-grid">
            {''.join(_render_bulletin_item(row) for row in bulletins) or '<article class="bulletin-card"><p>今天还没有达到快讯阈值的热点。</p></article>'}
          </div>
        </section>
        {section_blocks}
      </main>
...
def _render_radar_card(row) -> str:
    summary = clean_html(row["summary"], max_length=92) or "等待后续补充摘要。"
    return f"""
    <article class="radar-card">
      <strong>{escape(to_channel_intro(row['category'] or '其他'))}</strong>
...
def _render_archive_item(row) -> str:
    ...
    <li class="archive-item">
      <strong><a href="{escape(link)}">{escape(row['digest_date'])}</a></strong>
...
def _render_source_item(row) -> str:
    ...
    <li class="source-item">
      <strong>{escape(row['source_name'])}</strong>
      <span>收录 {row['article_count']} 条 · 最高热度 {row['max_score']}</span>
    </li>
    """


def _render_lead_item(row) -> str:
    section_name = SECTION_ORDER.get(row["category"] or "其他", "其他资讯")
    return _render_news_card("hero-card", section_name, row, "精选")


def _render_bulletin_item(row) -> str:
    section_name = SECTION_ORDER.get(row["category"] or "其他", "其他资讯")
    badge = "快讯" if row["is_breaking"] else "精选"
...
def _render_section(section_name: str, rows: list) -> str:
    ...
    return f"""
    <section id="{_slugify(section_name)}" class="section-card" data-section="{escape(section_name)}">
      <h2>{escape(section_name)}</h2>
      <div class="section-list">{stories}</div>
    </section>
    """
...
def _render_news_card(card_class: str, section_name: str, row, badge_label: str) -> str:
    summary = clean_html(row["summary"], max_length=170) or "等待后续补充摘要。"
    context = f'<div class="context">{escape(row["context_note"])}</div>' if row["context_note"] else ""
    why = f'<div class="why">看点：{escape(row["why_it_matters"])}</div>' if row["why_it_matters"] else ""
    source_name = str(row["source_name"] or "")
    theme = _source_theme(source_name)
    avatar = _avatar_text(source_name or row["title"] or "G")
    ...
    <article class="{card_class}" data-card data-section="{escape(section_name)}" data-search="{escape(_search_text(row))}" style="--card-accent:{theme['accent']}; --avatar-bg:{theme['avatar_bg']}; --avatar-fg:{theme['avatar_fg']};">
      ...
      <div class="news-card">
        ...
        <div class="news-main">
          <div class="news-head">
            <h3 class="news-title"><a href="{escape(row['url'] or '#')}" target="_blank" rel="noreferrer">{escape(row['title'])}</a></h3>
            <span class="score-pill">热度 {row['importance_score']}</span>
          </div>
          <div class="news-submeta">
            <span>{escape(source_name or '未知来源')}</span>
            <span>相关报道 {row['article_count']}</span>
            <span>{escape(to_channel_intro(row['category'] or '其他'))}</span>
          </div>
...
def _render_tags(row) -> str:
    tags: list[str] = []
    category = to_channel_intro(row["category"] or "其他")
...
def _source_theme(source_name: str) -> dict[str, str]:
    ...
    if "playstation" in lower or "/r/ps5" in lower:
        return {
            "accent": "#3b82f6",
            "avatar_bg": "linear-gradient(145deg, #0b2346 0%, #2e6fdd 100%)",
            "avatar_fg": "#ffffff",
        }
    if "xbox" in lower:
        return {
            "accent": "#189b52",
            "avatar_bg": "linear-gradient(145deg, #103221 0%, #2db36c 100%)",
            "avatar_fg": "#ffffff",
        }
    if "ign" in lower:
        return {
            "accent": "#d83b3b",
            "avatar_bg": "linear-gradient(145deg, #491515 0%, #d83b3b 100%)",
            "avatar_fg": "#ffffff",
        }
    if "gamespot" in lower:
        return {
            "accent": "#f59e0b",
            "avatar_bg": "linear-gradient(145deg, #4c2c08 0%, #f59e0b 100%)",
            "avatar_fg": "#ffffff",
        }
    if "reddit" in lower:
        return {
            "accent": "#ff6b35",
            "avatar_bg": "linear-gradient(145deg, #5f2410 0%, #ff6b35 100%)",
            "avatar_fg": "#ffffff",
        }
    if "rock paper shotgun" in lower:
        return {
            "accent": "#8b5cf6",
            "avatar_bg": "linear-gradient(145deg, #24153f 0%, #8b5cf6 100%)",
            "avatar_fg": "#ffffff",
        }
    return {
        "accent": "#1f6feb",
        "avatar_bg": "linear-gradient(145deg, #0e1726 0%, #29354d 100%)",
        "avatar_fg": "#ffffff",
    }
...
def _parse_digest_sections(content: str) -> list[tuple[str, list[dict[str, str]]]]:
    ...
    for raw_line in content.splitlines():
        ...
        if line.startswith("- **") and line.endswith("**"):
            ...
            continue
        if line.startswith("- ") and current_section == "今日导读":
            current_item = {"title": line[2:].strip()}
            section_map.setdefault(current_section, []).append(current_item)
            continue
